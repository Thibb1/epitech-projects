...
int p_process(int fd, va_list li, flag *flag, int count)
{
    ...
    for (; !flag->m && flag->wide > nb_size; flag->wide--)
        count += my_putchar(fd, flag->f);
    count += my_dprintf(fd, "%#x", va_arg(li, void *));
    for (; flag->m && flag->wide > nb_size; flag->wide--)
        count += my_putchar(fd, flag->f);
    return (count);
}
...
int b_process(int fd, va_list li, flag *flag, int count)
{
    ...
    for (; !flag->m && flag->wide > nb_size && flag->f == ' '; flag->wide--)
        count += my_putchar(fd, flag->f);
    count += hex_process(fd, flag, nb_size, 0);
    for (; !flag->m && flag->wide > nb_size; flag->wide--)
        count += my_putchar(fd, flag->f);
    count += b_put(fd, nb, flag->spe, 0);
    for (; flag->m && flag->wide > nb_size; flag->wide--)
        count += my_putchar(fd, flag->f);
    return (count);
}
...
int s_process(int fd, va_list li, flag *flag, int count)
{
    ...
    if ('s' == flag->spe)
        ...
        if (flag->dot)
            count += my_shownstr(fd, str, flag->precision);
        else
            count += my_showstr(fd, str);
    for (; flag->m && flag->wide > s_length; flag->wide--)
        count += my_putchar(fd, ' ');
    return (count);
}
